...
class ShowCreateTableSuite extends v1.ShowCreateTableSuiteBase with CommandSuiteBase {
  ...
  override def getShowCreateDDL(tableOrView: String, serde: Boolean = false): Array[String] = {
    super.getShowCreateDDL(tableOrView, serde).filter(!_.startsWith("'transient_lastDdlTime'"))
  }
