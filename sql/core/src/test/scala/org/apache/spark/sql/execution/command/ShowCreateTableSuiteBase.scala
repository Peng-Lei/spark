...
trait ShowCreateTableSuiteBase extends QueryTest with DDLCommandTestUtils {
  ...
  def getShowCreateDDL(tableOrView: String, serde: Boolean = false): Array[String] = {
    if (serde) {
      sql(s"SHOW CREATE TABLE $tableOrView AS SERDE").head().getString(0).split("\n").map(_.trim)
    } else {
      sql(s"SHOW CREATE TABLE $tableOrView").head().getString(0).split("\n").map(_.trim)
    }
  }
}
